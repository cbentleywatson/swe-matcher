...
const Basic_Questions = () => {
  ...
  const compatibility = useCallback((sender) => {
    /*The results need to be send right here */
    // e.preventDefault();
    const results = JSON.stringify(sender.data);
    console.log("Result!");
    ...
    axios.get('http://localhost:5000/compatibility/' + results)
      .then((response) => {
        //alert("in response");
        console.log("Sent From the test");
        console.log(JSON.stringify(response.data));
        ...
        var str = "";
        str = "\\\lll" //response.data;
        alert(str.replace(/\\/g, ''));
        var inJSON = JSON.parse(response.data);
        alert(inJSON.p1.FirstName);
        //    alert(response);
        //alert("axios response");
  ...
  */
  ...
  survey.onComplete.add(compatibility);
